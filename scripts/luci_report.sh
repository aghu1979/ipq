...
grep "^CONFIG_PACKAGE_luci.*=y$" "$POST_DIY_CONFIG_PATH" | sed 's/^CONFIG_PACKAGE_\(.*\)=y/\1/' | sort > "$POST_DIY_PACKAGES_FILE"
grep "^CONFIG_PACKAGE_luci.*=y$" "$FINAL_CONFIG_PATH" | sed 's/^CONFIG_PACKAGE_\(.*\)=y/\1/' | sort > "$FINAL_PACKAGES_FILE"

# 合并所有软件包并排序，用于生成表格的行
cat "$INITIAL_PACKAGES_FILE" "$POST_DIY_PACKAGES_FILE" "$FINAL_PACKAGES_FILE" | sort -u > "$ALL_PACKAGES_FILE"
...
# 生成报告
{
    ...
    while IFS= read -r pkg; do
        ...
        if grep -q "^${pkg}$" "$POST_DIY_PACKAGES_FILE"; then
            post_diy_status="✅"
        fi
        ...
        echo "| $pkg | $initial_status | $post_diy_status | $final_status |"
    ...
    echo "### 📝 变更分析"
    echo ""
    echo "- **DIY 脚本新增**: $(comm -13 "$INITIAL_PACKAGES_FILE" "$POST_DIY_PACKAGES_FILE" | wc -l) 个"
    echo "- **DIY 脚本移除**: $(comm -23 "$INITIAL_PACKAGES_FILE" "$POST_DIY_PACKAGES_FILE" | wc -l) 个"
    echo "- **自动依赖新增**: $(comm -13 "$POST_DIY_PACKAGES_FILE" "$FINAL_PACKAGES_FILE" | wc -l) 个"
    echo "- **自动依赖移除**: $(comm -23 "$POST_DIY_PACKAGES_FILE" "$FINAL_PACKAGES_FILE" | wc -l) 个"

} > "$REPORT_OUTPUT_PATH"

# 清理临时文件
rm -f "$INITIAL_PACKAGES_FILE" "$POST_DIY_PACKAGES_FILE" "$FINAL_PACKAGES_FILE" "$ALL_PACKAGES_FILE"
